...
func (r *RnicRegistry) RegisterRNIC(
	...
	// Current time in SQLite datetime format (space-separated, no timezone suffix)
	// MUST use this format (not RFC3339) for correct SQLite text comparison with datetime('now', ...)
	// RFC3339 uses 'T' as date-time separator (ASCII 84), while SQLite datetime() uses space (ASCII 32).
	// Since 'T' > space in lexicographic order, RFC3339 timestamps always compare as "greater"
	// within the same calendar day, making the TTL filter ineffective until UTC midnight.
	// This caused all registrations to expire simultaneously at UTC 00:05 (CST 08:05) every day.
