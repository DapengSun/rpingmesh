...
func (u *UDQueue) processSingleWC(cqEx *C.struct_ibv_cq_ex) {
	...
	switch gwc.Opcode {
	case C.IBV_WC_RECV:
		// handleRecvCompletion always returns true in normal flow because:
		// 1. ACK case: already reposted in handleRecvCompletion
		// 2. Successfully sent to recvCompChan: ReceivePacket will handle repost
		// 3. Channel full case: already reposted immediately to prevent RQ exhaustion
		// 4. Error cases: already reposted in error handling paths
		// So we should never need to repost here to avoid double posting
		...
		if !repostHandled {
			...
			log.Error().
				...
				Str("type", getQueueTypeString(u.QueueType)).
				Uint64("wr_id", gwc.WRID).
				Msg("handleRecvCompletion returned false unexpectedly. This should not happen. Skipping repost to avoid double posting.")
		}
	case C.IBV_WC_SEND:
...
func (u *UDQueue) handleRecvCompletion(gwc *GoWorkCompletion) bool {
	// Returns true if repost is already handled:
	// - ACK case: already reposted in handleRecvCompletion
	// - Channel full case: already reposted immediately to prevent RQ exhaustion
	// - Successfully sent to recvCompChan: ReceivePacket will handle repost
	// - Error cases: already reposted in error handling paths
	// This function should always return true in normal flow to avoid double posting
	log.Trace().
		Str("qpn", fmt.Sprintf("0x%x", u.QPN)).
		Str("type", getQueueTypeString(u.QueueType)).
	...
	if slot < 0 || slot >= u.NumRecvSlots {
		...
		return true // Already reposted (or attempted), return true to avoid double posting
	}
	...
	if u.QueueType == UDQueueTypeSender && u.ackHandler != nil && probePkt.IsAck == 1 {
		...
		case u.recvCompChan <- gwc:
			// Successfully sent to recvCompChan, ReceivePacket will handle repost
			// Return true to indicate repost is handled (by ReceivePacket)
			return true
		default:
			log.Warn().
				Str("qpn", fmt.Sprintf("0x%x", u.QPN)).
				Str("type", getQueueTypeString(u.QueueType)).
				Int("slot", slot).
				Msg("Receive completion channel full, dropping WC_RECV event. Attempting to repost receive buffer immediately to prevent queue exhaustion.")
			// Even if channel is full, we must repost the receive buffer to prevent receive queue exhaustion
			// This is critical to avoid ENOMEM errors (error code 12)
			if errPost := u.PostRecvSlot(slot); errPost != nil {
				log.Error().Err(errPost).
					Str("device", u.RNIC.DeviceName).
					Uint32("qpn", u.QPN).
					Int("slot", slot).
					Msg("CRITICAL: Failed to repost receive buffer after channel full. This may lead to receive queue exhaustion.")
			}
			// Return true because we already reposted here
			return true
		}
	}
	// This should not be reached in normal flow
	// All paths above should have returned true
	return true
}
