...
type UDQueue struct {
	...
	// Per-send completion channels to prevent stale completion contamination.
	// On timeout, the entry is intentionally left so the CQ poller can absorb
	// the eventual WC into the buffered channel, which is then GC'd.
	// key: WR-ID (uint64), value: chan *GoWorkCompletion (buffer=1)
	pendingSendChans sync.Map
}
