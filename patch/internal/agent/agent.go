...
func (a *Agent) updatePinglist() {
	...
	for _, localRnic := range localRnics {
		...
		successCount++ // [NEW] Increment success count
	...
	// [NEW] If all requests failed, log warning and keep existing targets (don't update)
	// This prevents clearing the pinglist when connection fails, allowing agent to continue with existing targets
	if successCount == 0 && errorCount > 0 {
		log.Warn().
			Str("feature", "pinglist_error_handling").
