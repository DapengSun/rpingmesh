...
// Returns (sendCompletionTime, error) where sendCompletionTime is the HW wallclock
// timestamp from the SEND WC (CompletionWallclockNS), used as t4.
...
//
// Parameters:
//   - receiveTime        (t3): HW wallclock from the RECV WC (CompletionWallclockNS).
//   - sendCompletionTime (t4): HW wallclock returned by SendFirstAckPacket.
func (u *UDQueue) SendSecondAckPacket(
	...
	packet.T4 = uint64(sendCompletionTime.UnixNano())
	packet.IsAck = 1
